...
async def download_media(
  ...
  media_name: str,
  took_at: str,  # Add this parameter
  verbose: bool,
) -> None:
  ...
  if not os.path.exists(destination_filename):
    ...
    # Set file modification time from tookAt
    if took_at:
      try:
        dt = datetime.datetime.fromisoformat(took_at)
        timestamp = dt.timestamp()
        os.utime(destination_filename, (timestamp, timestamp))
      except (ValueError, OSError) as e:
        if verbose:
          print(f"Warning: Could not set timestamp for {media_name}: {e}")
      ...
      for media in data["mediaFiles"]:
        ...
        download_coroutines.append(
          download_media(
            ...
            media["uuid"],
            media.get("tookAt"),  # Pass the tookAt timestamp
            args.verbose,
          )
        )
